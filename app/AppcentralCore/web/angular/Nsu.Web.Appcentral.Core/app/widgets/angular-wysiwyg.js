...
(function(angular, undefined) {
    ...
    var DEFAULT_MENU = [
        ['italic', 'underline','bold', 'strikethrough', 'subscript', 'superscript'],
        ['format-block'],
        ['font'],
        ['font-size'],
        ...
        .directive('wysiwyg', function($timeout, wysiwgGui, $compile) {
            return {
                template: '<div>' +
                    ...
                    '<div id="{{textareaId}}" ng-attr-style="resize:vertical;height:{{textareaHeight || \'80px\'}}; overflow:auto; word-break: break-all;" contentEditable="{{!disabled}}" class="{{textareaClass}} wysiwyg-textarea" rows="{{textareaRows}}" name="{{textareaName}}" required="{{textareaRequired}}" placeholder="{{textareaPlaceholder}}" ng-model="value"></div>' +
                    '</div>',
                restrict: 'A',
                scope: {
                    value: '=ngModel',
